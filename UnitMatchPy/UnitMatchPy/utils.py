...
def load_good_waveforms(wave_paths, unit_label_paths, param, good_units_only=True):
    ...
    if good_units_only:
        ...
        for ls in range(len(wave_paths)):
            # load in the first good unit, to get the shape of each waveform
            p_file = os.path.join(
                wave_paths[ls], f"Unit{int(all_units[ls][0])}_RawSpikes.npy"
            )
            tmp = np.load(p_file)
            tmp_waveform = np.zeros(
                (
            ...
            for i in range(len(os.listdir(wave_paths[ls]))):
                # loads in all GoodUnits for that session
                p_file_good = os.path.join(
                    wave_paths[ls], f"Unit{int(all_units[ls][i])}_RawSpikes.npy"
                )
                tmp_waveform[i] = np.load(p_file_good)
            # adds that session to the list
            waveforms.append(tmp_waveform)
    ...
    for ls in range(len(wave_paths)):
        ...
        for i in range(len(good_units[ls])):
            ...
            tmp_path_good = os.path.join(
                wave_paths[ls], f"Unit{int(good_units[ls][i].squeeze())}_RawSpikes.npy"
            )
...
def evaluate_output(
    ...
    for did in range(param["n_sessions"]):
        ...
        print(
            f"The percentage of false +ve's is {false_positive_est_per_session[did]}% for session {did +1}"
        )
...
def fill_missing_pos(KS_dir, n_channels):
    ...
    for x in x_unique:
        ...
        y_steps, y_step_counts = np.unique(
            np.diff(np.unique(same_y_pattern)), return_counts=True
        )
    ...
    if pos.shape[0] == n_channels - 1:
        ...
        channel_pos[np.isnan(channel_pos)[:, 0], :] = np.vstack(channel_pos_new)[
            missed_pos_idx
        ]
        return channel_pos
    else:
    ...
    if missed_pos_idxs.shape[0] == empty_idx.shape[0]:
        ...
        for idx in missed_pos_idxs:
            ...
            estimated_missing_idx = empty_idx[
                np.argmin(np.abs(empty_idx - nearest_idx))
            ]
            # fill in this index
...
def paths_from_KS(KS_dirs):
    ...
    wave_paths = []
    for i in range(n_sessions):
        # if os.path.exists(os.path.join(KS_dirs[i], "mean_waveforms_2split.npy")):
        #     wave_paths.append(os.path.join(KS_dirs[i], "mean_waveforms_2split.npy"))
    ...
    for i in range(n_sessions):
        ...
        n_channels.append(waveform_tmp.shape[2])
