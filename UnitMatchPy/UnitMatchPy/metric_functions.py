...
def re_scale(vector):
    ...
    """
    score = (np.nanquantile(vector, 0.99) - vector) / (
        np.nanquantile(vector, 0.99) - np.nanmin(vector)
    )
    score[score < 0] = 0
...
def get_wave_corr(avg_waveform, param):
    ...
    # make into a score
    wave_corr = (wave_corr - np.nanquantile(wave_corr, 0.005)) / (
        np.nanquantile(wave_corr, 0.995) - np.nanquantile(wave_corr, 0.005)
    )
...
def get_waveforms_mse(avg_waveform, param):
    ...
    projected_waveform_norm = (
        projected_waveform_norm - np.nanmin(projected_waveform_norm, axis=0)
    ) / (
        np.nanmax(projected_waveform_norm, axis=0)
        - np.nanmin(projected_waveform_norm, axis=0)
    )

    x1 = np.tile(
        np.expand_dims(projected_waveform_norm[:, :, 0], axis=2),
        (1, 1, projected_waveform_norm.shape[1]),
    )
    x2 = np.swapaxes(
        np.tile(
            np.expand_dims(projected_waveform_norm[:, :, 1], axis=2),
            (1, 1, projected_waveform_norm.shape[1]),
        ),
        1,
        2,
...
def flip_dim(avg_waveform_per_tp, param):
    ...
    avg_waveform_per_tp_flip = np.full(
        (3, n_units, spike_width, 2, len(flip_dim) + 1), np.nan
    )
    ...
    for i in range(len(flip_dim)):
        ...
        new_vals = (
            np.nanmin(tmpdat, axis=1, keepdims=True)
            + np.nanmax(tmpdat, axis=1, keepdims=True)
            - tmpdat
        )
...
def get_Euclidean_dist(avg_waveform_per_tp_flip, param):
    ...
    x1 = np.tile(
        np.expand_dims(avg_waveform_per_tp_flip[:, :, waveidx, 0, :], axis=-1),
        (1, 1, 1, 1, n_units),
    ).squeeze()
    x2 = np.swapaxes(
        np.tile(
            np.expand_dims(avg_waveform_per_tp_flip[:, :, waveidx, 1, :], axis=-1),
            (1, 1, 1, 1, n_units),
        ).squeeze(),
        1,
        4,
...
def centroid_metrics(euclid_dist, param):
    ...
    centroid_dist = np.nanmin(
        euclid_dist[:, new_peak_loc - waveidx == 0, :, :].squeeze(), axis=1
    ).squeeze()

    centroid_dist = 1 - (
        (centroid_dist - np.nanmin(centroid_dist))
        / (max_dist - np.nanmin(centroid_dist))
    )
    ...
    centroid_var = np.nanmin(
        np.nanvar(euclid_dist, axis=1, ddof=1).squeeze(), axis=1
    ).squeeze()
...
def get_recentered_euclidean_dist(avg_waveform_per_tp_flip, avg_centroid, param):
    ...
    avg_waveform_per_tp_fip_recenterd = np.swapaxes(
        np.swapaxes(avg_waveform_per_tp_flip, 2, 3) - avg_centroid_broadcast, 2, 3
    )
    x1 = np.tile(
        np.expand_dims(avg_waveform_per_tp_fip_recenterd[:, :, waveidx, 0, :], axis=-1),
        (1, 1, 1, 1, n_units),
    ).squeeze()
    x2 = np.swapaxes(
        np.tile(
            np.expand_dims(
                avg_waveform_per_tp_fip_recenterd[:, :, waveidx, 1, :], axis=-1
            ),
            (1, 1, 1, 1, n_units),
        ).squeeze(),
        1,
...
def dist_angle(avg_waveform_per_tp_flip, param):
    ...
    x2 = avg_waveform_per_tp_flip[
        :, :, waveidx[0] : waveidx[-2] + 1, :, :
    ]  # Difference between python and ML indexing
    ...
    for dim_id1 in range(avg_waveform_per_tp_flip.shape[0]):
        for dim_id2 in np.arange(1, avg_waveform_per_tp_flip.shape[0]):
            if dim_id2 <= dim_id1:
                continue
            ang = np.abs(x1[dim_id1, :, :, :, :] - x2[dim_id1, :, :, :, :]) / np.abs(
                x1[dim_id2, :, :, :, :] - x2[dim_id2, :, :, :, :]
            )

            loc_angle[:, :, :, :, count_id] = (
                np.arctan(ang) * good_angle
            )  # only selects angles for units where there is sufficient distance between time poitns
            count_id += 1

    loc_angle = np.nansum(loc_angle, axis=4)
    x1 = np.tile(np.expand_dims(loc_angle[:, :, 0, :], axis=-1), (1, 1, 1, n_units))
    x2 = np.swapaxes(
        np.tile(np.expand_dims(loc_angle[:, :, 1, :], axis=-1), (1, 1, 1, n_units)),
        0,
        3,
    )
    ...
    x2 = np.swapaxes(
        np.tile(np.expand_dims(traj_dist[:, :, 1, :], axis=-1), (1, 1, 1, n_units)),
        0,
        3,
    )
...
def get_threshold(total_score, within_session, euclid_dist, param, is_first_pass=True):
    ...
    thrs_opt = score_vector[
        np.argwhere(
            (pf.smooth(hd, 3) > pf.smooth(hnd, 3)) * (score_vector > 0.6) == True
        )
    ][0]
    # if ThrsOpt.size == 0:
...
def get_good_matches(pairs, total_score):
    ...
    for pair_id in range(2):
        ...
        tmp_vals = idx[
            ls
        ]  # returns the unit idx for PairID, where there is more than one potential match
        ...
        for i in range(len(tmp_vals)):
            ...
            tmp_pair = np.argwhere(
                pairs[:, pair_id] == tmp_vals[i]
            )  # idx of pair for the multiple matched unit
            tmp = pairs[
                tmp_pair, :
            ].squeeze()  # unit idx pair, for each double match e.g (2,272) and (2,278)
...
def drift_correction_basic(
    candidate_pairs, session_switch, avg_centroid, avg_waveform_per_tp
):
    """
    ...
    idx = np.argwhere(
        (
            (best_pairs[:, 0] < session_switch[1])
            * (best_pairs[:, 1] >= session_switch[1])
        )
        == True
    )

    drift = np.nanmedian(
        np.nanmean(avg_centroid[:, best_pairs[idx, 0].squeeze(), :], axis=2)
        - np.nanmean(avg_centroid[:, best_pairs[idx, 1].squeeze(), :], axis=2),
        axis=1,
    )
...
def apply_drift_correction_basic(
    pairs, sid, session_switch, avg_centroid, avg_waveform_per_tp
):
    """
    ...
    drift = np.nanmedian(
        np.nanmean(avg_centroid[:, pairs[:, 0], :], axis=2)
...
def apply_drift_correction_per_shank(
    pairs, sid, session_switch, avg_centroid, avg_waveform_per_tp, param
):
    """
    ...
    for i in range(n_shanks):
        ...
        correct_shank_a = np.logical_and(
            centroid_a[1, :] < max_dist, centroid_a[1, :] > min_dist
        )
        correct_shank_b = np.logical_and(
            centroid_b[1, :] < max_dist, centroid_b[1, :] > min_dist
        )

        if np.all(correct_shank_a == correct_shank_b) != True:
            print(
                f"These pairs may be bad {np.argwhere(correct_shank_a != correct_shank_b)}"
            )
...
def shank_ID_per_session(avg_centroid, session_switch, sid, param):
    ...
    centroid_pos = np.nanmean(
        avg_centroid[:, session_switch[sid] : session_switch[sid + 1], :], axis=2
    )
    ...
    for i in range(n_shanks):
        ...
        shank_idx = np.logical_and(
            centroid_pos[1, :] < max_dist, centroid_pos[1, :] > min_dist
        )
...
def test_matches_per_shank(pairs, avg_centroid, sid, param):
    ...
    if n_shanks != 1:
        ...
        if np.any(counts < units_per_shank_thrs):
            ...
            print(
                f"Session pair {sid+1}/{sid+2} has {counts} matches per shank, which is below threshold to do per shank drift correction"
            )
    else:
        do_per_shank_correction = False
...
def drift_n_sessions(
    ...
    for did in range(n_sessions - 1):
        idx = np.argwhere(
            (
                (best_pairs[:, 0] >= session_switch[did])
                * (best_pairs[:, 0] < session_switch[did + 1])
                * (best_pairs[:, 1] >= session_switch[did + 1])
                * (best_pairs[:, 1] < session_switch[did + 2])
            )
            == True
        )
        ...
        if (
            test_matches_per_shank(pairs, avg_centroid, did, param) == True
            and best_drift == True
        ):
            drifts = np.zeros((n_sessions - 1, param["no_shanks"], 3))
            drifts[did, :, :], avg_waveform_per_tp, avg_centroid = (
                apply_drift_correction_per_shank(
                    pairs, did, session_switch, avg_centroid, avg_waveform_per_tp, param
                )
            )
            print(
                f"Done drift correction per shank for session pair {did+1} and {did+2}"
            )
