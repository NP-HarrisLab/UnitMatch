...
def get_max_sites(waveform, channel_pos, clus_info, param):
    ...
    for i in range(
        channel_pos[0].shape[0]
    ):  # looping over each site, assuming all channel_pos, are the same no of channels
        for j in range(n_units):
            dist = np.linalg.norm(
                channel_pos[session_id[j]][max_site_mean[j], :]
                - channel_pos[session_id[j]][i, :]
            )
    ...
    waveform_filt = (
        mask * waveform
    )  # applying the filter, so only allow good time points/spatial points

    spatial_fp_filt = get_spatial_fp(waveform_filt)

    max_site = get_max_site(
        spatial_fp_filt
    )  # This is the max site of each individual cv
...
def decay_and_average_waveform(
    ...
    for i in range(n_units):
        ...
        for cv in range(2):
            dist_to_max_chan = np.linalg.norm(
                good_pos - channel_pos[session_id[i]][max_site[i, cv]], axis=1
            )
            ...
            d_10[i, cv] = (
                tmp_min  # distance to where the amplitude decay to 10% of its peak
            )
            ...
            for s in range(n_channels):  # looping over each site
                ...
                dist = np.linalg.norm(
                    channel_pos[session_id[i]][max_site_mean[i], :]
                    - channel_pos[session_id[i]][s, :]
                )
                ########################################################################################################
            ...
            spatial_fp = np.max(
                np.abs(waveform[i, :, tmp_idx.astype(bool), cv]), axis=1
            )
            spatial_fp = np.expand_dims(spatial_fp, axis=-1)
            mu = np.sum(np.tile(spatial_fp[:], (1, 3)) * loc, axis=0) / np.sum(
                spatial_fp[:]
            )
            ...
            dist_to_max_proj = np.linalg.norm(
                loc - avg_centroid[:, i, cv].T, axis=1
            )  # it is transposed here
            weight = (d_10[i, cv] - dist_to_max_proj) / d_10[i, cv]
            avg_waveform[:, i, cv] = np.nansum(
                waveform[i, :, tmp_idx.astype(bool), cv].T
                * np.tile(weight, (spike_width, 1)),
                axis=1,
            ) / np.sum(weight)

            # significant time points
            wave_duration_tmp = np.argwhere(
                np.abs(avg_waveform[:, i, cv]) - np.mean(avg_waveform[0:20, i, cv])
                > 2.5 * np.std(avg_waveform[0:20, i, cv], axis=0)
            )
            if wave_duration_tmp.size == 0:
                wave_duration_tmp = waveidx
            wave_duration_tmp = wave_duration_tmp[
                np.isin(wave_duration_tmp, waveidx)
            ]  # okay over achiever, gonna cut you off there
            if wave_duration_tmp.size == 0:
                wave_duration_tmp = waveidx
            ...
            peak_time[i, cv] = np.argmax(
                np.abs(
                    avg_waveform[
                        wave_duration_tmp[0] : wave_duration_tmp[-1] + 1, i, cv
                    ]
                )
            )
        ...
        for cv in [0, 1]:
            ...
            if peak_time[i, 0] != new_peak_loc:  # yes the first CV
                ...
                if shift != 0:
                    ...
                    waveform[i, :, :, cv] = np.roll(
                        waveform[i, :, :, cv], shift, axis=0
                    )
...
def get_avg_waveform_per_tp(
    ...
    for i in range(n_units):
        for cv in range(2):
            ...
            wave_duration_tmp = np.argwhere(
                np.abs(np.sign(amplitude[i, cv]) * avg_waveform[waveidx, i, cv])
                > (np.sign(amplitude[i, cv]) * amplitude[i, cv] * 0.25)
            )

            if wave_duration_tmp.sum != 0:
                try:
                    wave_duration_tmp = (
                        np.arange(wave_duration_tmp[0], wave_duration_tmp[-1] + 1)
                        + waveidx[0]
                    )
                    ...
                    print(
                        f"unit{i} is very likely bad, no good time points in average waveform"
                    )
                    waveform_duration[i, cv] = np.nan

            else:
                print(
                    f"problem with active time points (waveform_duration) for unit {i}"
                )
    ...
    for uid in range(n_units):
        for cv in range(2):
            for dim in range(avg_waveform_per_tp.shape[0]):
                avg_waveform_per_tp[
                    dim, uid, good_wave_idxs[uid, :, cv].astype(bool), cv
                ] = gaussian_filter(
                    avg_waveform_per_tp[
                        dim, uid, good_wave_idxs[uid, :, cv].astype(bool), cv
                    ],
