...
def runUMSet( KS_dirs, custom_unit_label_paths, save_dir, est_drift=np.asarray([]) ):
    ...
    if np.any(est_drift):
        for i in range(n_sess-1):
            ...
            print(curr_drift)            
            drift, avg_waveform_per_tp, avg_centroid = mf.apply_drift_correction_basic(None, i, session_switch, avg_centroid, avg_waveform_per_tp, curr_drift)
    ...
    prior_match = 1 - ( param['n_expected_matches'] / len(include_these_pairs))     # these have high 
    ...
    util.evaluate_output(output_prob_matrix, param, within_session, session_switch, match_threshold = param['match_threshold'])
...
def main():
    ...
    drift_est_all = np.zeros((len(day_list)-1,3))  # drift bewteen sessions, distance from shank, x, z
            ...
            for sh_ind in sh_list:
                for rs1 in rs_list:
                    ...
                    for ind1 in range(len(day_list)):
                        ...
                        curr_ks_path = os.path.join(ks_parent,f'{rec_list[id1-1]}_imec{sh_ind}',ks_name)
                    ...
                    for ind2 in range(ind1+1,len(day_list)):                    
                        for rs1 in rs_list:
                            ...
                            if not os.path.exists(save_dir):
                                os.mkdir(save_dir)
                            print(f'save dir: {save_dir}') 
                            runUMSet( [ks_dir_1, ks_dir_2], [label_path_1, label_path_2], save_dir )
